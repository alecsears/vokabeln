...
<div class="flex items-center justify-between mb-5">
  <h1 class="text-2xl font-bold" style="color:var(--text)">👥 Nutzer</h1>
</div>
...
<div class="grid grid-cols-2 gap-4 mb-6">
  <?php foreach ($users as $u): ?>
    ...
    <a href="/profile.php?user=<?= htmlspecialchars($u['id']) ?>"
       class="card p-4 flex flex-col items-center gap-2 text-center hover:opacity-80 transition no-underline">
      ...
      <div class="font-bold" style="color:var(--text)"><?= htmlspecialchars($u['name']) ?></div>
      <div class="text-xs text-muted">
        ✅ <?= (int)($us['total_correct'] ?? 0) ?> &nbsp;❌ <?= (int)($us['total_wrong'] ?? 0) ?>
      </div>
      <div class="text-xs" style="color:var(--primary)">Profil öffnen →</div>
    </a>
  <?php endforeach; ?>
</div>
...
<div class="card p-4">
  ...
  <form method="post">
    ...
    <button type="submit" class="btn btn-primary w-full">Erstellen</button>
  </form>
</div>
